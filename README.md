...
После аренды VPS-сервера первым делом проверяем, есть ли доступ к auth и world серверам (**обязательно** проверяем и то, и другое). Смотрим сначала, идет ли пинг, затем — есть ли подключение по TCP. Если всё в порядке — можно приступать к деплою.
...
Смотрим вниз на значение пинга (включая mdev) и packet loss (должен быть 0%).
```
